...
vidcap = cv2.VideoCapture('bw_nature.mp4')
success,image = vidcap.read()
count = 1
success = True
while success:
  cv2.imwrite("Video_images/%d.tiff" % count, image)    
  img = Image.open("Video_images/%d.tiff" % count)
  img = img.resize((512,256))
  img.save("Video_images/%d.tiff" % count, "TIFF", optimize=True)
  success, image = vidcap.read()
  count += 1
  if(count > 12):
      break  
...
def collect_natural_patches(num_patches = 100000, patch_width = 8):
  ...
  nat_img_cnt = 1  
  image = PIL.Image.open('Video_images/' + str(nat_img_cnt) + '.tiff')
